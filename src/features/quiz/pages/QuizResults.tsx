...
import { CheckIcon, XIcon, RefreshIcon } from '@/components/ui/Icons';
import { getScoreColorClass } from '../utils/quizScoring';
...
export const QuizResults: React.FC<QuizResultsProps> = ({ quiz, userAnswers, score, onRestart }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-surface dark:bg-surface p-8 rounded-2xl shadow-xl animate-fade-in border border-border dark:border-border">
      <h2 className="text-3xl font-bold text-center mb-4 bg-gradient-to-r from-primary-light via-primary-light to-primary-light-hover dark:from-primary dark:via-primary dark:to-primary-hover bg-clip-text text-transparent animate-fade-in">
        Quiz Results
      </h2>
        ...
        <div className="w-48 h-48 rounded-full flex items-center justify-center bg-gradient-to-br from-background dark:from-background to-surface dark:to-surface shadow-inner">
          <div className="w-40 h-40 rounded-full flex items-center justify-center bg-surface dark:bg-surface shadow-md">
            <p
              ...
              style={{ ...({} as React.CSSProperties), textShadow: '2px 2px 8px rgba(0,0,0,0.3)' }}
            >
        ...
        You answered{' '}
        {
          userAnswers.filter(
            (a) =>
              ...
              quiz.questions[a.questionIndex]?.correctAnswerIndex === a.selectedOption
          ).length
        }{' '}
      ...
      <div className="space-y-6">
        {quiz.questions.map((question, index) => {
          ...
          return (
            <div
              key={index}
              className={`p-4 rounded-lg border-l-4 transition-all animate-slide-up ${
                isCorrect ? 'bg-green-500/10 border-green-500' : 'bg-red-500/10 border-red-500'
              }`}
              style={{ animationDelay: `${index * 0.05}s` }}
            >
              <p className="font-semibold text-text dark:text-text-DEFAULT mb-3">
                ...
                {question.options.map((option, optionIndex) => {
                  ...
                  return (
                    <div key={optionIndex} className={itemClass}>
                      {isCorrectAnswer ? (
                        <CheckIcon className="h-5 w-5 mr-2 flex-shrink-0 text-green-400" />
                      ) : isUserAnswer ? (
                        <XIcon className="h-5 w-5 mr-2 flex-shrink-0 text-red-400" />
                      ) : (
                        <div className="w-5 h-5 mr-2 flex-shrink-0" />
                      )}
                      <span className="flex-grow">{option}</span>
                      {isCorrectAnswer && isUserAnswer === null && (
                        <span className="text-xs font-bold text-yellow-500 ml-2">(unanswered)</span>
                      )}
                    </div>
                  );
                })}
