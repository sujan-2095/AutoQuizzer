...
import { ChevronLeftIcon, ChevronRightIcon, ClockIcon, ExclamationIcon } from '@/components/ui/Icons';
...
export const QuizTaker: React.FC<QuizTakerProps> = ({ quiz, onQuizSubmit, setIsDirty }) => {
  ...
  return (
    <div className="relative max-w-3xl mx-auto bg-surface dark:bg-surface p-6 sm:p-8 rounded-2xl shadow-xl animate-fade-in border border-border dark:border-border">
      ...
      <h2 className="text-2xl sm:text-3xl font-bold text-center mb-4 bg-gradient-to-r from-primary-light via-primary-light to-primary-light-hover dark:from-primary dark:via-primary dark:to-primary-hover bg-clip-text text-transparent animate-fade-in">
        {quiz.title}
      </h2>

      <div className="mb-6">
        <div className="flex justify-between mb-1">
          <span className="text-base font-medium text-primary-light dark:text-primary">
            Question {currentQuestionIndex + 1} of {quiz.questions.length}
          </span>
          <span className="text-sm font-medium text-primary-light dark:text-primary">
            {Math.round(progress)}%
          </span>
        </div>
        <div className="w-full bg-surface-light dark:bg-surface-light rounded-full h-2.5">
          <div
            className="bg-primary-light dark:bg-primary h-2.5 rounded-full transition-all duration-500"
