...
import { ChartPieIcon, DownloadIcon, PlayIcon, TrashIcon, ExclamationIcon } from '@/components/ui/Icons';
import Modal from '@/components/ui/Modal';
import { getScoreColorClass } from '@/features/quiz/utils/quizScoring';
...
export const QuizCard: React.FC<QuizCardProps> = ({ quiz, onTakeQuiz, onDeleteQuiz, onDownloadPdf }) => {
            ...
            <div className="border-t border-border dark:border-border pt-4 mt-4 space-y-3 text-sm text-text-light-muted dark:text-text-muted">
              ...
              {typeof quiz.lastScore === 'number' && (
                <div className="flex items-center">
                  ...
                  <span className={`font-bold text-base ${getScoreColorClass(quiz.lastScore)}`}>
        ...
        <div>
          ...
          <div className="flex justify-end space-x-3 mt-6">
            <button
              ...
              className="px-4 py-2 text-sm font-semibold text-text-light-muted dark:text-text-muted hover:bg-surface-light dark:hover:bg-surface-light rounded-lg transition-all duration-300 transform hover:scale-105"
            >
              Cancel
            </button>
            <button
              onClick={handleDeleteConfirm}
              className="px-4 py-2 text-sm font-semibold text-white bg-danger hover:bg-danger-hover rounded-lg transition-all duration-300 transform hover:scale-105"
            >
              Delete
            </button>
          </div>
